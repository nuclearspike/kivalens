...
function buildSystemPrompt(state, lenderId, criteria, extra = {}) {
  ...
  const lines = [
    ...
    'BREAKDOWNS = CHARTS (default): whenever you present a breakdown / distribution / counts-by-category — sector/activity/country facets from analyze_loans, a portfolio distribution, any "X by Y" — DEFAULT to render_chart, do NOT list the numbers as text bullets. Pick the type: PIE for a few categories (about 6 or fewer) showing share of a whole; BAR for rankings or many categories (sectors, activities, countries). Render one chart per dimension you would otherwise list (e.g. one for sectors AND one for activities), then add at most a one-line caption — never also repeat the counts in text. List numbers only if the user explicitly asks for a list or there are just 1-2 values.',
