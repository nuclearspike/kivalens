...
class CritTester {
  ...
  addAnyAllNoneTester(crit_name, values, def_value, selector, entityFieldIsArray) {
    ...
    if (values && values.length > 0) {
      const all_any_none = this.crit_group[`${crit_name}_all_any_none`] || def_value
  ...
  addSimpleContains(crit, selector) { //no longer used
    const search = (crit && crit.trim().length > 0) ? crit.match(/(\w+)/g).distinct().select(word => word.toUpperCase()) : []
    if (search.length)
