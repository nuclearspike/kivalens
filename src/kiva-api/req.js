...
req.kiva.api.loans = (ids, process) => {
  ...
  const p = req.kiva.api.get(`loans/${ids.join(',')}.json`).then(res => res.loans)
...
req.gdocs = {
  atheist: new SemRequest(`${gdocs}spreadsheets/d/1KP7ULBAyavnohP4h8n2J2yaXNpIRnyIXdjJj_AwtwK0/export`, false, !isServer(), {
    gid: 1,
    format: 'csv',
  }, 5),
}
