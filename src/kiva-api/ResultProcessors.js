...
const common_use = ["PURCHASE", "FOR", "AND", "BUY", "OTHER", "HER", "BUSINESS", "SELL", "MORE", "HIS", "THE", "PAY"]
const common_descr = common_use.concat(["THIS", "ARE", "SHE", "THAT", "HAS", "LOAN", "BE", "OLD", "BEEN", "YEARS", "FROM", "WITH", "INCOME", "WILL", "HAVE"])
...
class ResultProcessors {
  ...
  static unprocessLoan(loan) {
    ...
    Object.keys(l).filter(f => f.indexOf('kl_') === 0).forEach(field => delete l[field])
  ...
  static processLoan(loan) {
    ...
    if (loan.kls) { // replace what was stripped out by the server before sending down.
      ...
      loan.borrowers = loan.borrowers.concat(Array.range(1, loan.klb.F || 0).select(x => ({
        gender: "F",
        first_name: '...',
      })))
    ...
    if (loan.description.texts) { //the presence implies this is a detail result; this doesn't run during the background refresh.
      ...
      loan.kl_disbursal_in_days = function () {
        return (new Date(loan.terms.disbursal_date) - Date.now()) / (24 * 60 * 60 * 1000)
      }.bind(loan)
      ...
      if (loan.terms.scheduled_payments && loan.terms.scheduled_payments.length) {
        ...
        if (!loan.kls)
          loan.terms.scheduled_payments = loan.terms.scheduled_payments.groupBy(p => p.due_date).select(g => ({
        ...
        let nextDate = new Date(Math.min(Date.next().month().set({day: 1}).clearTime(), repayments.first().date)).clearTime()
      ...
      loan.description.languages.filter(lang => lang !== 'en').forEach(lang => delete loan.description.texts[lang])
      ...
      if (loan.borrowers) //only visible
        loan.borrowers.filter(b => b.last_name === '').forEach(b => delete b.last_name)
