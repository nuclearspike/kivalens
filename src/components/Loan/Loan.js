...
const Loan = ({id, tab}) => {
  ...
  const basketItem = useSelector(({basket}) => basket.first((bi) => bi.id === id))
  ...
  return (
    <div className="Loan">
      <h1 style={{marginTop: '10px'}}>
        <Link to={`/search`}>Search</Link> > <KivaLink title="View loan on Kiva.org"
                                                       path={`lend/${loan.id}`}>{loan.name}</KivaLink>
        {inBasket ? (
          <Button
            variant="danger"
      ...
      <div className="margin-bottom-20">
        <b>{loan.location.country} | {loan.sector} | {loan.activity}</b>
        <br/>{loan.use}
      </div>
